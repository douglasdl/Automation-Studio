...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="createStyle">
    ...
    <xsl:if test="$useWidgetIndex='false' or $isWidgetInUse='true' or $itemType='system'">
      ...
      <!-- itemName of related widget for e.g. path to .widget file -->
      <xsl:variable name="widgetName">
        <xsl:choose>
          <xsl:when test="$itemType='structuredProperty'">
            <xsl:call-template name="get-item-name">
              <xsl:with-param name="xsiType" select="$relatedWidget"></xsl:with-param>
            </xsl:call-template>
          </xsl:when>
          <xsl:otherwise>
  ...
  <xsl:template name="createEntry">
    ...
    <xsl:choose>
      <xsl:when test="$itemType='structuredProperty'">
        <!-- a .widget file containing StructuredProperties can have several StyleProperties tags -->
        <!-- therefore we have to specify which StructuredProperty we need -->
        <xsl:for-each select="document($typeFilePath)//widget:StructuredProperty[@name=$itemName]//widget:StyleProperty[@name=$propertyName]/widget:StyleElement">
          <xsl:choose>
  ...
  <xsl:template name="getTypeFilePath">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$itemType='structuredProperty'">
        <xsl:value-of select="concat($basepath,$itemPath,$elpathdelimiter,'meta',$elpathdelimiter,$widgetName,'.widget')"/>
      </xsl:when>
      <xsl:otherwise>
