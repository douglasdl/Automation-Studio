...
function (Client, contentHelper, tooltipDependency, Utils, Types, Enum, DialogQueue) {
    ...
    */
    ...
    p.init = function (runtimeService, injectedContentHelper) {
        if (_runtimeService !== undefined) {
            _runtimeService.removeEventListener('action', this.serverActionHandler);
        }
        if (injectedContentHelper) {
            _contentHelper = injectedContentHelper;
        }
        _runtimeService = runtimeService;
        _runtimeService.addEventListener('action', this.serverActionHandler);
    ...
    function _runOpenDialogAction(action) {
        ...
        if (run) {
            ...
            $.when(brease.overlayController.openDialog(args.dialogId, args.mode, args.horizontalPos, args.verticalPos, undefined, args.headerText, args.autoClose)).then(function (success) {
    ...
    function _runOpenDialogAtTarget(action) {
        ...
        if (run) {
            ...
            if (target !== null) {
                $.when(brease.overlayController.openDialogAtTarget(args.dialogId, args.mode, args.horizontalPos, args.verticalPos, target, args.headerText, args.autoClose, args.horizontalDialogAlignment, args.verticalDialogAlignment)).then(function (success) {
